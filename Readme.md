...
In order to understand better how deep neural networks have to be designed and trained for facial recognition, different architectures have been implemented and the influence of different parameters and strategies during training have been studied.
...
In order to perform the recognition task each one of the images passes through a **Siamese Network that share all the weights** while working in tandem on the two different images. A [Neural Network pretrained for image classification](https://pytorch.org/docs/stable/torchvision/models.html#classification) have been used  for the Siamese's Networks. With the features extracted from the Siamese's Networks, two different strategies are explored:
1. Use a Decision Network to classify the two images as belonging to the same or different identity.
2. Use a Loss Function that compresses intra-variance (same identity) and enlarges inter-variance (different identities) of the features vectors.
...
Next, it's included , as an example, the code sniped with the implemented model at version 2 (all the versions are described in paragraph: How to reproduce the experiments).
...
Below, it's included, as an example, the code sniped with the implemented model at version 2 (all the versions are described in paragraph: How to reproduce the experiments).
...
| 23 | 2 | CosineEmbeddingLoss | VGG16's Convolutionals | 1e-4 | 14 | Not | Yes | Yes | **0.821**  | 12 | 0.807 
| 23 | 2 | CosineEmbeddingLoss | VGG16's Convolutionals | 5e-4 | 14 | Not | Yes | Yes | 0.709 | 14 | 0.801
...
When training the network with a learning rate of 5e-4 or 1e-3, some problems for convergence with the Adam Optimizer have been encountered. For that reason, the SGD Optimizer is used and it works better in that particular cases. Bigger learning rates speed up the training, but you have to be careful that it's not too big.
...
| 23 | 2 | CosineEmbeddingLoss | VGG16's Conv + Linear Layer | 1e-4 | Adam | 40 | Not | Yes | Yes | 0.832 | 17 | -
| [23](https://drive.google.com/open?id=1YwPhbOXRJE0WGSNrHaviHakgxJJ4xRRL) | 2 | CosineEmbeddingLoss | VGG16's Conv + Linear Layer | 1e-3 | SGD | 40 | Not | Yes | Yes | 0.835 | 28 | 0.641
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMzM5MjMyMTAsMTQxNDIwNzg1LC0xMD
c0ODU3NDIwLDE4NjIyOTQxNzNdfQ==
-->